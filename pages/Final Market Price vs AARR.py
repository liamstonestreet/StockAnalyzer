...
fig.add_trace(go.Scatter(
    ...
    text=[f"Max AARR: {y_max_aarr:.1f}%<br>@ ${x_max_aarr:.2f}"],
...
if strike < initial_price:
    ...
    st.write(f"- ✅ But the ${premium:.2f}/share premium makes up for it and then some")
    
if x_breakeven_vs_hold > initial_price:
    st.write(f"- 📈 Covered call beats holding if stock stays below **${x_breakeven_vs_hold:.2f}**")
else:
    st.warning("⚠️ Holding stock outperforms this call at most realistic price points")
    
st.write(f"- 🛡️ Downside protection: Stock can drop to **${x_zero:.2f}** before you lose money")
